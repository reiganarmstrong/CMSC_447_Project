...
class levelThreeScene extends Scene {
  ...
  preload() {
    ...
    this.load.image("level3", "assets/png/LEVEL3.png");
  ...
  create() {
    ...
    this.level_graphic = this.add.image(512, 250, "level3");
    this.small_level_graphic = this.add.image(950, 30, "level3");
        ...
        diveBombTimeline.add({
          ...
          onUpdate: () => {
            ...
            if (started == 1) {
              ...
              //testing stuff
              if (
                start_y + 50 >= diveBomber.y &&
                diveBomber.y >= start_y - 50
              ) {
                ...
                diveBomber.setVelocityX(
                  Math.pow(1, side) *
                    300 *
                    Math.cos(
                      2 * (diveBomber.body.rotation + 0) * (Math.PI / 180)
                    )
                );
              ...
              diveBomber.setVelocityX(
                Math.pow(1, side) *
                  300 *
                  Math.cos(2 * (diveBomber.body.rotation + 0) * (Math.PI / 180))
              );
              if (diveBomber.y > 800) {
                diveBomber.y -= 850;
              }
  ...
  update() {
    ...
    if (
      this.ship_health != 0 &&
      this.time_remaining != 0 /*&& !this.scene.isPaused*/
    ) {
  ...
  checkGameOver() {
    if (this.ship_health == 0) {
      ...
      this.scene.pause("levelThreeScene");
    }
  }
